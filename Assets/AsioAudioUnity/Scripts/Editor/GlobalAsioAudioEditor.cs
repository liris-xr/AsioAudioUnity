...
public class GlobalAsioAudioEditor : EditorWindow
{
    ...
    static void ConvertAudioSource(AudioSource audioSourceToConvert)
    {
        ...
        {
            Debug.LogWarning("AudioSource " + audioSourceToConvert.name + " has no AudioClip attached, the AudioFilePath of Custom ASIO Audio Source will be set to null.");
        }
        else
        {
        ...
        if (audioSourceToConvert.loop) audioSourceToConvertGameObject.GetComponent<CustomAsioAudioSource>().Loop = true;
        else audioSourceToConvertGameObject.GetComponent<CustomAsioAudioSource>().Loop = false;

        if (audioSourceToConvert.playOnAwake) audioSourceToConvertGameObject.GetComponent<CustomAsioAudioSource>().PlayOnAwake = true;
        else audioSourceToConvertGameObject.GetComponent<CustomAsioAudioSource>().PlayOnAwake = false;
    ...
    [MenuItem("GameObject/AsioAudioUnity/Custom ASIO Audio Source", false, -22)]
    static void CreateCustomAsioAudioSource(MenuCommand menuCommand)
    {
        CreateCustomAsioAudioSource(menuCommand, false);
    }

    [MenuItem("GameObject/AsioAudioUnity/Custom ASIO Audio Source (with Source Position OSC)", false, -21)]
    ...
    [MenuItem("GameObject/AsioAudioUnity/ASIO Audio Manager", false, 5)]
    ...
    [MenuItem("GameObject/AsioAudioUnity/OSC Manager", false, 6)]
